...
| Schwab trading | `k8s/apps/schwab/go-schwab-feed/files/schwab-trading.json` | 1360 | TBD (`phase-7-port-schwab-trading` queued) |
| Schwab Argo Rollouts | `k8s/apps/schwab/go-schwab-feed/files/schwab-rollouts.json` | 290 → 547 | **platform-dashboards phase-4** (ported, single-PR cutover) |
...
| **platform-dashboards phase-4** | Port `schwab-rollouts.json` (6 panels), add `state-timeline` + `barchart` panel-type SDK usage + `stat` value-mapping (6 mappings), promote `$namespace` from hidden constant to query variable, ride along the 4 deferred 7e3 palette-classic fixes, retag `[chalupa, platform, argo-rollouts]` (drop `canary`/`schwab`/`rollouts`) | **shipped** |
...
| schwab-rollouts | 6 | 4 multi-series panels missing palette-classic; `$namespace` is a `constant` variable (not user-pickable — intentional? unclear) | Trivial in source, but JSON uses compact-style nested objects → round-trip patch produces +237 lines of formatting churn for 4 substantive lines. Skip inline. | **resuperseded by SDK port** (`platform-dashboards phase-4`): palette-classic shipped on panels 3–6; `$namespace` promoted to single-value query variable backed by `label_values(rollout_phase, exported_namespace)` |
